import { atomSource, defineKey, PET_TICK_PATCH_PATHS, stateSource } from '../define';
...
export const GARDEN_KEYS = {
  ...
  myGarden: defineKey<Record<string, unknown>>({
    policy: 'authoritative', tier: 'state', doc: 'My garden subtree (tileObjects, boardwalkTileObjects, ...) — wakes only on garden patches',
    sources: [
      stateSource('/child/data/userSlots/{myIdx}/data/garden', (s, id) => {
        const g = selectMyData(s, id)?.garden;
        return isRecord(g) ? g : undefined;
      }, { trustPatches: true }),
